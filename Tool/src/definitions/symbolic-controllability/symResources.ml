  ...
  | mon::mons ->
    (match mon with
      ...
      | Ast.Monitor.TVar(x) -> 
        ...
        then fv_mon mons (Vars.union var_set (fv_tvar x var_set ([x] @ tvars_checked))) ([x] @ tvars_checked)
...
| x::xs -> 
  print_all xs; 
  print_endline (print_identifier_string x)
...
| Some m -> 
  ...
  | Some n -> 
    print_endline("it has some free vars");
    print_all (Vars.elements n);
...
let rec rename_monvar (m: Ast.Monitor.t) (bound: BoundTVars.t): Ast.Monitor.t = 
  ...
  | Ast.Monitor.Recurse(x) -> 
    ...
    then
      ...
      (* in 
      mapTVarFv := TVars.add (print_tvar_string new_tvar) free_vars !mapTVarFv;
      print_endline("free vars are ");
      print_all(Vars.elements free_vars);
      exit(0); *)
      ...
      (* let free_vars = fv ([], [x.consume]) Vars.empty
      in  *)
      (* mapTVarFv := TVars.add x.monvar.tvar free_vars !mapTVarFv; *)
  ...
  let counter = 0 in
    let init_f = "$" in
      let rec generateFrsh (last_f: Ast.Identifier.t) (counter: int): Ast.Identifier.t =
        ...
        then (print_endline(print_identifier_string last_f); last_f)
        else (
